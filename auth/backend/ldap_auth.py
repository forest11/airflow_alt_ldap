        ...
        for resp in conn.response:
            if (
                    'attributes' in resp and (
                        ...
                        username in [x.split(",")[0].split("=")[1] for x in resp['attributes'].get(group_member_attr) ]
...
class LdapUser(models.User):
    ...
    def user_mail(username):
        ...
        res = conn.search(native(configuration.get("ldap", "user_basedn")),
                          native(search_filter),
                          search_scope=native(SUBTREE),
                          attributes=[native("mail")]
                          )

        if not res:
            LOG.warning("Unable to find user for %s", username)
        else:
            for resp in conn.response:
                if 'attributes' in resp and resp['attributes'].get("mail"):
                    return resp['attributes'].get("mail")
